...
//缓冲运动的运动框架并且可以取任意值的运动框架
        startMove:function(obj,target,attr){

//若元素有定时器则先清除，若无则加上这个属性
        if(obj.timer){ clearInterval(obj.timer);}
        else{obj.timer = null}

        var that =this;//非常重要在定时器内部，this指针是指向，window的
...
//style代表当前样式的属性值
                ...
                if(attr == "opacity"){
                    ...
                    style = that.getStyle(obj,attr)*100||1;
...
//opacity:0.1的小数，在电脑内存中小数及其容易出错，所以一定要避免
                    ...
                    style = parseInt(that.getStyle(obj,attr))|| 0;
                ...
                if(style == target){
                    ...
                    if(attr == "opacity"){
                        obj.style[attr] =  (style*100 +speed)/100;
                        //兼容ie
...
//dom操作
        ...
        getChildByClassname:function(node,classname){
        var childNodes = this.getChild(node);
        var result = [];
        var nodeClassName=[];
        //如果没有孩子节点返回null
...
//未找到元素时候，返回空数组
        ...
        removeClassName:function(node,Name){
        ...
        //用于替换部分字符串
        replaceStrPart:function(str,targetStr,replaceStr){
        str = str.split("/");
         ...
         * sucess,表示回调函数
         *         */
        getAjax:function(method,url,success,data){
        var xml;
